...
trips <- read_excel("Lithuania_DATA_2018-2019-2020_winter_trials.xlsx", sheet="tbl_trip", skip=1)
sets <- read_excel("Lithuania_DATA_2018-2019-2020_winter_trials.xlsx", sheet="tbl_set", skip=1)
gear <- read_excel("Lithuania_DATA_2018-2019-2020_winter_trials.xlsx", sheet="tbl_gear", skip=1)
bycatch <- read_excel("Lithuania_DATA_2018-2019-2020_winter_trials.xlsx", sheet="tbl_bycatch")
fish <- read_excel("Lithuania_DATA_2018-2019-2020_winter_trials.xlsx", sheet="tbl_fish")
...
totfish<-fish %>% group_by(Trip_ID, Set_ID, Net_modification) %>% summarise(catch=sum(Weight)/1000) %>%
  rename(Cont_Treat=Net_modification)

DATA<-sets %>% select(-Depl_Time,-Haul_time,-Hours_deployed,-Days_deployed,-Fishing_depth,-Fishing_block,
-`FOR large Vesels  WP...12`,-`FOR large Vesels  WP...13`,-lat_start,-long_start,-lat_end,-long_end) %>%
  left_join(gear, by="Set_ID") %>%
  left_join(totfish, by=c('Trip_ID', 'Set_ID', 'Cont_Treat')) %>%
...
controls<- DATA %>% filter(Cont_Treat=="Control") %>% mutate(CPUE=ifelse(is.na(CPUE),0,CPUE))
LEDs<- DATA %>% filter(Cont_Treat=="Lights18/19BW")

## bootstrap test
boot.samples <- matrix(sample(controls$CPUE, size = 10000 * nrow(controls), replace = TRUE),10000, nrow(controls))
boot.statistics <- apply(boot.samples, 1, mean)
RATE_control<-data.frame(treatment="no LED",mean=mean(boot.statistics),
                         lcl=quantile(boot.statistics,0.025),ucl=quantile(boot.statistics,0.975))
boot.samples <- matrix(sample(LEDs$CPUE, size = 10000 * nrow(LEDs), replace = TRUE),10000, nrow(LEDs))
boot.statistics <- apply(boot.samples, 1, mean)
RATE_LED<-data.frame(treatment="with LED",mean=mean(boot.statistics),
                     lcl=quantile(boot.statistics,0.025),ucl=quantile(boot.statistics,0.975))
